...
contract SmartVotingSystem{
    ...
    uint256 public noOfCandidates; // records no of candidates nominated
    uint256 public totalVotes; // records total no of votes
    address public admin;
    bool public voting;  // boolean variable for verifying if voting has ended or not
    ...
    function addCandidate(string memory _candName, string memory _candSymbol, uint256 _candAge) external payable
    returns(uint256,string memory,uint256,string memory){
        require(msg.value == 100,"Nomination fee is 0.01 ether");
